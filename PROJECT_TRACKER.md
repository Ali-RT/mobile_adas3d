...
| M24 | H1 v2 tiny-overfit workflow | Preflight rerun pending | Preserve the two-logit Core ML head while adding a fixed implicit background logit for DETR-style softmax supervision, ordinary-IoU quality targets, controlled negative quality weighting, and a resumable 16-image/400-step memorization gate with hard score, IoU, and count checks. The first Colab attempt exposed and fixed the shared preflight's hard-coded Chen counts; full runs retain strict 3712/3769 defaults and only the frozen tiny config permits identical 16/16 splits. Distillation remains false. |
