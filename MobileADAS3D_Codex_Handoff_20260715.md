...
#### 2026-08-31 A3 Conv Large completed and rejected
...
## R2b result; refinement family closed (2026-09-02)

The complete R2b gate preserved frozen R0 behavior but did not deliver the required Pedestrian improvement. Vehicle moderate 3D/BEV AP changed `-0.00096/+0.00010`; Pedestrian moderate 3D/BEV AP improved `+0.04435/+0.04960`. Pedestrian nearby recall changed only `0.68342→0.68563` (`+0.00220`), and localization-failure rate changed `0.24603→0.24471` (reduction `0.00132`), versus required `0.02/0.02`. Vehicle nearby recall remained stable at `0.88239`. The checkpoint SHA-256 is `a51b1cca6efc755a2096db33f53d93c7b9d8bbcbae6efbd0857667f5701de3f0`; `selected=null` and `full_run_authorized=false`.

Do not extend R2b or start another local Pedestrian refinement variation. R1, R2, and R2b close this family. The next task is a versioned governance decision: either commission a materially new accuracy architecture, or freeze R0 epoch 185 as the compression parent and use strict relative R0-preservation gates while retaining `0.80` Pedestrian nearby recall as an unmet aspirational product target. No external qualification or compression claim should begin before that contract is explicit.

## M51 frozen and M52 FP16 gate prepared (2026-09-10)

`R0_COMPRESSION_CONTRACT.md` now freezes epoch 185/hash as the immutable compression parent. A candidate must preserve at least 95% of every R0 moderate 3D/BEV AP metric, lose no more than one absolute percentage point of either nearby recall, add no more than one absolute percentage point of Pedestrian localization failures, and produce all 3,769 validation files. These are preservation gates only; Pedestrian nearby recall `0.80` remains unmet.

The first M52 attempt failed before the first validation batch. The native CUDA `vectorized_gather_kernel` assertions occur when the blanket-FP16 depth path produces an invalid discrete index for MonoDETR's depth-position embedding; the asynchronous traceback surfaces at the next transformer allocation, before deformable attention runs. No checkpoint was changed. The first correction isolated depth prediction and the pinned float-only custom deformable-attention extension in FP32, but its smoke test reproduced the same assertion. That proved the FP32 boundary began too late: backbone features and input projections had already been created under FP16 autocast before entering the depth island.

The revised M52 policy keeps backbone feature extraction, every input projection feeding depth, depth prediction, weighted-depth interpolation, depth-position embedding, and the legacy deformable-attention CUDA extension in FP32. Only remaining compatible transformer and prediction-head operators use FP16 autocast. The mandatory one-batch CUDA smoke now requires finite outputs, at least one FP16 output, FP32 backbone and projected features, a recorded FP32 depth-predictor output, and FP32 inputs at every observed custom-attention kernel before full validation. If this expanded smoke still fails, reject M52 on the legacy MonoDETR stack and move to the next compression approach rather than adding more ad hoc precision islands.

Restart the Colab runtime first because a device-side assertion poisons the active CUDA context, then run `notebooks/MonoDETR_M52_R0_FP16_Gate_Colab.ipynb` top-to-bottom. This revision uses the fresh path `/content/MonoDETR_M52_FP32_FEATURES` so an older partially patched checkout cannot be reused. It copies the exact R0 checkpoint into an isolated Drive tree, performs no training, and runs complete AP/nearby/localization evaluation only after the smoke test passes. Return `m52_fp16_gate_comparison.json` or CSV. A pass authorizes only the next compression experiment.

## M52 selective mixed precision completed and passed (2026-09-10)

The expanded one-batch CUDA smoke passed, so the notebook completed the full Chen validation over all 3,769 images using immutable R0 epoch 185 and checkpoint SHA-256 `fc0eba200e44b88921af76b0a5c94279872fd5c4838ab4d8936838447debfa59`. M52 Vehicle/Pedestrian moderate 3D AP_R40 was `17.6399/5.6952`, balanced moderate 3D mean was `11.6676`, and Vehicle/Pedestrian moderate BEV AP_R40 was `23.6395/6.6603`. Relative to R0, those changes were `+0.0052/-0.0261/-0.0105/-0.0421/+0.0642` AP respectively.

Vehicle/Pedestrian nearby recall was `0.88215/0.68519`, compared with R0 `0.88246/0.68342`. Pedestrian localization-failure rate improved slightly from `0.24603` to `0.24559`. All five AP floors, both recall floors, the localization ceiling, and the 3,769-file completeness gate passed. `compression_rung_authorized=true`; `product_safety_qualified=false` because the aspirational `0.80` Pedestrian nearby-recall target remains unmet.

The run's evaluation stage took about 18 minutes 33 seconds for 236 batches, but the evaluator's printed "images" count represents batches and there is no paired FP32 wall-time, latency-distribution, peak-memory, or power measurement. Treat M52 as an accuracy-preserving numerical-compatibility pass only; do not claim a speedup or deployability benefit from this result.
...
## M53 Colab smoke compatibility correction (2026-09-10)

The first M53 smoke attempt stopped before model construction. MonoDGP's upstream PyTorch-version condition evaluates the 2.x case incorrectly and selects the removed private `torch.nn.modules.linear._LinearWithBias`; the adjacent legacy `torch._overrides` branch has the same risk. This is a runtime compatibility failure, not checkpoint corruption, CUDA-extension failure, or an accuracy result.

The pinned, fail-closed patch now maps those two imports to public PyTorch APIs while preserving the model graph and weights. It was verified on a fresh copy of commit `aa059a18214aebf644510e7f0793971b403f9d14`, on a second idempotent patch pass, by Python compilation, and by the full 236-test repository suite. In the same live Colab runtime, rerun the notebook's source/setup cell and then its M53 smoke cell; the dataset links, checkpoint download, and prepared manifest do not need to be repeated. If the runtime restarted, run the notebook top-to-bottom. Start the complete 3,769-image evaluation only after the smoke passes.

## M53 native reference result and gate correction (2026-09-10)

The corrected smoke passed and official inference generated the exact 3,769-file Chen validation set. MonoDGP's native KITTI evaluator reported Car 3D AP_R40 `30.1185/22.6797/19.3705` for easy/moderate/hard, versus published `30.1314/22.7109/19.3978`. Absolute differences are only `0.0129/0.0312/0.0273`, so the native like-for-like reproduction passes every frozen `0.5`-AP gate.

The MobileADAS3D independent evaluator reported `29.8114/22.1628/18.7457`, lower than native by `0.3071/0.5169/0.6248` AP. That implementation remains useful for internally consistent diagnostics, but it is not the evaluator that generated MonoDGP's published benchmark and must not be the pass/fail authority for reproducing those numbers. The first schema-v1 report therefore produced a false gate failure despite a successful native reproduction.

The schema-v2 gate parses the durable MonoDGP native `train.log.*`, uses native AP for published-result tolerance checks, records independent AP separately, and automatically reuses a complete prediction set plus native log. Pull the correction and rerun the notebook's final cell; no checkpoint download or 3,769-image inference rerun is required. M54 remains blocked until the regenerated `m53_monodgp_reference_gate.json` reports `reference_reproduced=true` and `two_class_adaptation_authorized=true`.

## M53 completed and M54 authorized (2026-09-10)

The regenerated schema-v2 report passed every frozen M53 gate. It reused the complete 3,769-image prediction set and native evaluator log, retained the pinned upstream commit, exact checkpoint SHA-256 `1d5f30b34b8bef49638079a8b07f05ebf11bb5f85d6a9a11c7b028c69396f05d`, and exact Chen validation split provenance. Native official-evaluator Car 3D AP_R40 is `30.1185/22.6797/19.3705` for easy/moderate/hard, versus published `30.1314/22.7109/19.3978`; the absolute differences are `0.0129/0.0312/0.0273`, comfortably inside the frozen `0.5`-AP tolerance.
...
comparison CSV. The first conversion uses the unchanged FP32 runtime semantics
