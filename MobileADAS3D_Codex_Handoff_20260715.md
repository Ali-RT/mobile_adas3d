...
Run `notebooks/MonoDETR_M52_R0_FP16_Gate_Colab.ipynb` top-to-bottom on a Colab GPU. It clones pinned MonoDETR into `/content/MonoDETR_M52`, adds a config-controlled CUDA FP16-autocast inference path, copies the exact R0 checkpoint into an isolated Drive tree, performs no training, and runs complete AP/nearby/localization evaluation. Return `m52_fp16_gate_comparison.json` or CSV. A pass authorizes only the next compression experiment.
