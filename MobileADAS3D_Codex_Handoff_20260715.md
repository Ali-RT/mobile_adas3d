...
The first M52 attempt failed before the first validation batch. The native CUDA `vectorized_gather_kernel` assertions occur when the blanket-FP16 depth path produces an invalid discrete index for MonoDETR's depth-position embedding; the asynchronous traceback surfaces at the next transformer allocation, before deformable attention runs. No checkpoint was changed. M52 now uses selective mixed precision: the surrounding compatible graph uses FP16 autocast, while depth prediction, weighted-depth interpolation, and depth-position embedding execute inside an explicit FP32 island. The pinned custom deformable-attention CUDA extension is also isolated in FP32 because its dispatch supports float/double rather than half. The notebook adds a mandatory one-batch CUDA smoke test that requires finite outputs, at least one FP16 output, a recorded FP32 depth-predictor output, and FP32 inputs at every observed custom attention kernel before full validation.
