...
from scripts.evaluate_monodetr_a2e_gate import is_eligible
from scripts.patch_monodetr_small_pedestrian_box_weight import replace_once
from scripts.prepare_monodetr_a2e_gate import A2_EPOCH, A2_SHA256, VARIANTS


ROOT = Path(__file__).resolve().parents[1]
NOTEBOOK = ROOT / "notebooks/MonoDETR_A2e_Small_Pedestrian_Box_Gate_Colab.ipynb"


class MonoDETRA2eGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_four_variants_are_frozen(self):
        ...
        self.assertIn("--small-height-px','64'", self.code)

    def test_default_seed_survives_monodetr_squared_seed(self):
        source = (ROOT / "scripts/prepare_monodetr_a2e_gate.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("default=20268", source)
        self.assertLessEqual(20268**2, 2**32 - 1)

    def test_patch_weights_only_matched_small_native_pedestrian_pairs(self):
        source = (ROOT / "scripts/patch_monodetr_small_pedestrian_box_weight.py").read_text(encoding="utf-8")
        self.assertIn("target_labels == 0", source)
        self.assertIn("target_box_heights <= self.small_pedestrian_box_height_norm", source)
