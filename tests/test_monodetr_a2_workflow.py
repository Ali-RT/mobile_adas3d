...
from scripts.prepare_monodetr_a2_student import (
    R0_CHECKPOINT_SHA256,
    resolve_r0_selection,
)
from scripts.sweep_monodetr_a2_product_checkpoints import GATES


ROOT = Path(__file__).resolve().parents[1]
NOTEBOOK = ROOT / "notebooks/MonoDETR_A2_MobileNetV4_Medium_Two_Class_GT_Colab.ipynb"


class MonoDETRA2WorkflowTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_notebook_has_frozen_two_class_gt_only_contract(self):
        self.assertIn("prepare_monodetr_a2_student.py", self.code)
        self.assertIn("R0_SELECTION", self.code)
        self.assertIn("monodetr_a2_mnv4_vehicle_pedestrian_gt", self.code)
    ...
    def test_sweep_uses_a2_source_provenance(self):
        source = (
            ROOT / "scripts/sweep_monodetr_a2_product_checkpoints.py"
        ).read_text(encoding="utf-8")
        self.assertIn('"--source-name-prefix"', source)
        self.assertIn('"MobileMonoDETR_A2"', source)
    ...
    @mock.patch("scripts.prepare_monodetr_a2_student.sha256_file")
