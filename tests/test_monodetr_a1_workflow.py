...
from scripts.prepare_monodetr_a1_student import (
    R0_CHECKPOINT_SHA256,
    resolve_r0_selection,
)
from scripts.sweep_monodetr_a1_product_checkpoints import GATES


ROOT = Path(__file__).resolve().parents[1]
NOTEBOOK = ROOT / "notebooks/MonoDETR_A1_MobileNetV4_Two_Class_GT_Colab.ipynb"


class MonoDETRA1WorkflowTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_notebook_has_frozen_two_class_gt_only_contract(self):
        self.assertIn("prepare_monodetr_a1_student.py", self.code)
        self.assertIn("R0_SELECTION", self.code)
        self.assertIn("monodetr_a1_mnv4_vehicle_pedestrian_gt", self.code)
    ...
    def test_notebook_has_durable_exact_resume_and_sweep(self):
        ...
        self.assertIn("a1_product_selection.json", self.code)
    ...
    def test_sweep_uses_a1_source_provenance(self):
        source = (
            ROOT / "scripts/sweep_monodetr_a1_product_checkpoints.py"
        ).read_text(encoding="utf-8")
        self.assertIn('"--source-name-prefix"', source)
        self.assertIn('"MobileMonoDETR_A1"', source)
    ...
    @mock.patch("scripts.prepare_monodetr_a1_student.sha256_file")
