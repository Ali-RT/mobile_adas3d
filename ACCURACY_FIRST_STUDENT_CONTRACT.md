...
R1 result: rejected. The 2× Pedestrian Hungarian localization-cost branch improved nearby recall by only `0.00044` and reduced localization failures by only `0.00176`, far below the locked `0.02/0.02` requirements. Vehicle 3D changed `-0.15445` AP, narrowly exceeding the maximum allowed loss, and Pedestrian 3D/BEV both regressed. No full R1 is authorized, and further scalar matcher/loss weighting is closed. The next decision must explicitly choose structural Pedestrian refinement or governance of a relative compression-preservation gate; R0 still does not meet the `0.80` Pedestrian product target.

R2 is the single authorized structural gate. It retains R0 transformer inputs and outputs, and reads frozen ResNet layer-1 stride-4 features only through a new 1×1 projection and 3×3 per-query box sampler. A zero-initialized residual head adjusts only 2D left/right/top/bottom values, softly gated by native Pedestrian probability; projected 3D center, depth, dimensions, yaw, classification, transformer, queries, data, and decoding are unchanged at initialization. The paired control and R2 treatment use the same R0 checkpoint, five epochs, `1e-5`, and seed `20268`, with the same locked R1 eligibility thresholds.
