...
VARIANTS = {
    "control_refine_off": False,
    "ped_refine_stride4": True,
}
...
def create_refinement_initialization(repo: Path, config: dict, r0: Path, output: Path, seed: int) -> dict:
    ...
    if disallowed or unexpected or not missing:
        raise RuntimeError(
            f"Unsafe R2 initialization: missing={missing}, unexpected={unexpected}"
        )
    ...
    torch.save(
        {
            ...
            "r2_initialization": {
                ...
                "policy": "exact R0 state plus deterministic zero-output Pedestrian refinement",
            },
        },
        output,
    )
    return {"checkpoint": str(output), "checkpoint_sha256": sha256_file(output), "new_parameters": missing}


def main() -> None:
    parser = argparse.ArgumentParser(description="Prepare paired R2 stride-4 Pedestrian refinement gate.")
    ...
    for name, enabled in VARIANTS.items():
        ...
        run_name = f"monodetr_r2_{name}_gate{args.gate_epochs}"
        ...
        pretrain = r0
        if enabled:
            init_path = output_root / "initialization/r2_ped_refinement_init.pth"
    ...
    manifest = {
        "schema_version": 1,
        "complete": True,
        "experiment": "R2 paired stride-4 Pedestrian local-feature refinement gate",
        "controlled_change": "zero-initialized local-feature residual for predicted Pedestrian 2D box edges",
    ...
    path = output_root / "r2_gate_manifest.json"
