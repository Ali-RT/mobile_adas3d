...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the restartable product sweep and apply A1 accuracy gates."
    )
    ...
    gate_results = {name: float(selected[name]) >= threshold for name, threshold in GATES.items()}
    report = {
        "schema_version": 1,
    ...
    report_path = args.output_dir / "a1_product_selection.json"
