...
def is_eligible(row: dict, control: dict) -> bool:
    return bool(
        row["variant"] != "control_refine_off"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the paired R2 refinement gate.")
    ...
    control = next(row for row in rows if row["variant"] == "control_refine_off")
    for row in rows:
    ...
    (output_root / "r2_gate_comparison.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    with (output_root / "r2_gate_comparison.csv").open("w", newline="") as handle:
