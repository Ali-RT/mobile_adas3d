...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sweep A2g checkpoints and apply the frozen product gates."
    )
    ...
    raw_selection_path = args.output_dir / "r0_product_selection.json"
    raw_selection = json.loads(raw_selection_path.read_text(encoding="utf-8"))
    selected = raw_selection["selected"]
    gate_results = {
        name: float(selected[name]) >= threshold for name, threshold in GATES.items()
    }
    report = {
        "schema_version": 1,
        "complete": bool(raw_selection.get("complete")),
        ...
        "raw_selection": str(raw_selection_path),
    }
    report_path = args.output_dir / "a2g_product_selection.json"
