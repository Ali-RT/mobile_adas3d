...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Prepare the GT-only two-class MobileNetV4 MonoDETR A1 run."
    )
    ...
    parser.add_argument("--run-name", default="monodetr_a1_mnv4_vehicle_pedestrian_gt")
    ...
    init_checkpoint = run_dir / "checkpoint_a1_init.pth"
    torch.save(
        {
            "epoch": 0,
    ...
    config_output = (
        ...
        else repo / "configs/monodetr_a1_mnv4_vehicle_pedestrian_gt.yaml"
    )
    ...
    manifest = {
        "schema_version": 1,
        "experiment": "MobileMonoDETR-Student-A1 GT-only two-class baseline",
