...
def model_macs(model: torch.nn.Module) -> int:
    from ptflops import get_model_complexity_info

    macs, _ = get_model_complexity_info(
        MobileADAS3DS1TupleWrapper(model),
        (3, 384, 1280),
        as_strings=False,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Trace, convert, and validate the random-weight MobileADAS3D-S1 graph."
    )
    parser.add_argument(
        "--config",
        default="configs/kitti_mobileadas3d_s1.yaml",
    )
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument(
        "--predict",
        action="store_true",
        help="Compile and predict with Core ML on the current Mac.",
    )
    ...
    trace_path = output_dir / "MobileADAS3D_S1_random.pt"
    package_path = output_dir / "MobileADAS3D_S1_random.mlpackage"
    ...
    if args.predict:
        ...
        coreml_delta = max(
            float(
                np.max(
                    np.abs(
                        prediction[name].astype(np.float32)
    ...
    report_path = output_dir / "mobileadas3d_s1_coreml_gate.json"
    report_path.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(report, indent=2))
    if not complete:
        raise RuntimeError("MobileADAS3D-S1 random graph gate failed")
