...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Prepare the GT-only two-class MobileNetV4 MonoDETR A2 run."
    )
    ...
    parser.add_argument("--run-name", default="monodetr_a2_mnv4_vehicle_pedestrian_gt")
    ...
    init_checkpoint = run_dir / "checkpoint_a2_init.pth"
    torch.save(
        {
            "epoch": 0,
    ...
    config_output = (
        ...
        else repo / "configs/monodetr_a2_mnv4_vehicle_pedestrian_gt.yaml"
    )
    ...
    manifest = {
        "schema_version": 1,
        "experiment": "MobileMonoDETR-Student-A2 GT-only two-class baseline",
