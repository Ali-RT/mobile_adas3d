...
def is_eligible(row: dict, control: dict) -> bool:
    return bool(
        row["variant"] != "control_match_w1_0"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the paired R1 matcher gate.")
    ...
    for name, variant in manifest["variants"].items():
        ...
        rows.append(
            {
                "variant": name,
                "pedestrian_localization_cost_weight": variant["pedestrian_localization_cost_weight"],
    ...
    control = next(row for row in rows if row["variant"] == "control_match_w1_0")
    for row in rows:
    ...
    (output_root / "r1_gate_comparison.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    with (output_root / "r1_gate_comparison.csv").open("w", newline="") as handle:
