...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the paired A2f gate.")
    ...
    control = next(row for row in rows if row["variant"] == "control_stride8")
    for row in rows:
    ...
    (output_root / "a2f_gate_comparison.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    with (output_root / "a2f_gate_comparison.csv").open("w", newline="") as handle:
