...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the paired A2e gate.")
    ...
    for name, variant in manifest["variants"].items():
        ...
        rows.append(
            {
                "variant": name,
                "small_pedestrian_box_loss_weight": variant["small_pedestrian_box_loss_weight"],
    ...
    (output_root / "a2e_gate_comparison.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    with (output_root / "a2e_gate_comparison.csv").open("w", newline="") as handle:
