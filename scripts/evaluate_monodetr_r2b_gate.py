...
def is_eligible(row: dict) -> bool:
    return bool(
        ...
        and row["pedestrian_localization_reduction_vs_r0"] >= 0.02
        and row["vehicle_near_recall"] >= SOURCE_R0["vehicle_near_recall"] - 0.01
        and row["vehicle_3d_delta_vs_r0"] >= -0.15
        and row["vehicle_bev_delta_vs_r0"] >= -0.15
        and row["pedestrian_3d_moderate"] >= SOURCE_R0["pedestrian_3d_moderate"]
        and row["pedestrian_bev_moderate"] >= SOURCE_R0["pedestrian_bev_moderate"]
    )


def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the bounded R2b frozen-refinement gate.")
    ...
    for name, variant in manifest["variants"].items():
        ...
        rows.append(
            {
                "variant": name,
                "pedestrian_refinement_enabled": variant["pedestrian_refinement_enabled"],
                "gate_mode": "hard",
                "base_model_frozen": True,
                **{key: float(metrics[key]) for key in SOURCE_R0 if "recall" not in key and "failure" not in key},
    ...
    for row in rows:
        row["pedestrian_near_gain_vs_r0"] = row["pedestrian_near_recall"] - SOURCE_R0["pedestrian_near_recall"]
        row["pedestrian_localization_reduction_vs_r0"] = SOURCE_R0["pedestrian_localization_failure_rate"] - row["pedestrian_localization_failure_rate"]
        row["vehicle_3d_delta_vs_r0"] = row["vehicle_3d_moderate"] - SOURCE_R0["vehicle_3d_moderate"]
        row["vehicle_bev_delta_vs_r0"] = row["vehicle_bev_moderate"] - SOURCE_R0["vehicle_bev_moderate"]
    ...
    report = {
        ...
        "source_r0": SOURCE_R0,
        "eligibility_rule": {
            "pedestrian_near_gain_vs_r0_min": 0.02,
            "pedestrian_localization_reduction_vs_r0_min": 0.02,
    ...
    (output_root / "r2b_gate_comparison.json").write_text(
        json.dumps(report, indent=2) + "\n", encoding="utf-8"
    )
    with (output_root / "r2b_gate_comparison.csv").open("w", newline="") as handle:
