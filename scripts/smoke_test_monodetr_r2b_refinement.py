...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="R2b frozen hard-gated real-data parity and gradient preflight.")
    ...
    for inputs, calibs, raw_targets, _ in loader:
        mask = raw_targets["mask_2d"]
        if bool((raw_targets["labels"][mask] == 0).any()):
            break
    else:
        raise RuntimeError("No native Pedestrian target found for R2b smoke test")
    ...
    if not finite_gradients or gradient_sum <= 0:
        raise RuntimeError(
            f"R2b refinement gradient failed: finite={finite_gradients}, sum={gradient_sum}"
        )
    report = {
        "schema_version": 1,
        "complete": True,
        ...
        "gate_mode": treatment.pedestrian_refinement_gate_mode,
        "base_model_frozen": treatment.pedestrian_refinement_freeze_base,
        "optimizer_steps": 0,
    }
