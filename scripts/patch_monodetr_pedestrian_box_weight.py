...
def main() -> None:
    ...
    replace_once(
        ...
        "    def __init__(self, num_classes, matcher, weight_dict, focal_alpha, losses, group_num=11, pedestrian_box_loss_weight=1.0):",
        "criterion Pedestrian box-weight argument",
    )
    replace_once(
        model,
        "        self.focal_alpha = focal_alpha\n        self.ddn_loss = DDNLoss()",
        "        self.focal_alpha = focal_alpha\n"
        "        self.pedestrian_box_loss_weight = float(pedestrian_box_loss_weight)\n"
        "        if self.pedestrian_box_loss_weight < 1.0:\n"
        "            raise ValueError('pedestrian_box_loss_weight must be >= 1.0')\n"
        "        self.ddn_loss = DDNLoss()",
        "criterion Pedestrian box-weight state",
    )
    replace_once(
        model,
        ...
        "            target_labels == 0,\n"
        "            pair_weights * self.pedestrian_box_loss_weight,\n"
        "            pair_weights)\n\n"
        "        # l1",
        "matched Pedestrian pair weights",
    )
    replace_once(
        model,
    ...
    replace_once(
        ...
        "        focal_alpha=cfg['focal_alpha'],\n"
        "        losses=losses,\n"
        "        pedestrian_box_loss_weight=cfg.get('pedestrian_box_loss_weight', 1.0))",
        "criterion Pedestrian box-weight config",
    )
    print("MonoDETR matched-Pedestrian 2D box weighting patch ready")
