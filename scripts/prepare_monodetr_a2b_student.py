...
def create_pedestrian_balanced_split(dataset_root: Path, repeat: int) -> tuple[str, dict]:
    ...
    return split_name, {
        ...
        "balanced_samples": len(balanced_ids),
        "balanced_split": str(output),
        "balanced_split_sha256": sha256_file(output),
    }
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Prepare the A2b pedestrian-balanced GT-only MobileNetV4 MonoDETR run."
    )
    ...
    parser.add_argument(
        "--run-name", default="monodetr_a2b_mnv4_pedestrian_balanced_gt"
    )
    parser.add_argument("--pedestrian-repeat", type=int, default=2)
    ...
    train_split, sampling = create_pedestrian_balanced_split(
        dataset_root, args.pedestrian_repeat
    )
    ...
    init_checkpoint = run_dir / "checkpoint_a2b_init.pth"
    torch.save(
        {
            "epoch": 0,
    ...
    config_output = (
        ...
        else repo / "configs/monodetr_a2b_mnv4_pedestrian_balanced_gt.yaml"
    )
    ...
    manifest = {
        "schema_version": 1,
        "experiment": "MobileMonoDETR-Student-A2b pedestrian-balanced GT-only",
        ...
        "changed_component": "training sampling only relative to frozen A2",
