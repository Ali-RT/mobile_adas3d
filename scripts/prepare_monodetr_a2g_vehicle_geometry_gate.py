...
def resolve_a2_checkpoint(selection_path: Path) -> tuple[Path, dict]:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Prepare paired A2g Vehicle 3D geometry fine-tune gates."
    )
    ...
    checkpoint, selection = resolve_a2_checkpoint(args.a2_selection.resolve())
    ...
    for name, multiplier in VARIANTS.items():
        ...
        run_name = f"monodetr_a2g_{name}_gate{args.gate_epochs}"
        config["model_name"] = run_name
        config["random_seed"] = args.seed
        original_coefficients = {}
        for key in GEOMETRY_KEYS:
            if key not in config["model"]:
                raise KeyError(f"Base config is missing model.{key}")
            original = float(config["model"][key])
            original_coefficients[key] = original
            config["model"][key] = original * multiplier
        ...
        variants[name] = {
            ...
            "effective_coefficients": {
                key: float(config["model"][key]) for key in GEOMETRY_KEYS
            },
    ...
    manifest = {
        ...
        "controlled_change": (
            "multiply only 3D-center, dimension, angle, and depth loss coefficients; "
            "2D losses, matching, sampling, backbone, and architecture remain unchanged"
        ),
        "a2_epoch": A2_EPOCH,
        "a2_checkpoint": str(checkpoint),
        "a2_checkpoint_sha256": A2_SHA256,
        "a2_selection": str(args.a2_selection.resolve()),
    ...
    manifest_path = output_root / "a2g_gate_manifest.json"
