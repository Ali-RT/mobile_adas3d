...
def main() -> None:
    ...
    prediction_dir = Path(manifest["run_dir"]) / "outputs/data"
    if prediction_dir.exists():
        shutil.rmtree(prediction_dir)
    run(
        [sys.executable, "-u", "tools/train_val.py", "--config", manifest["runtime_config"], "--evaluate_only"],
        monodgp,
    )

    val_ids = [line.strip() for line in val_split.read_text(encoding="utf-8").splitlines() if line.strip()]
    prediction_ids = {path.stem for path in prediction_dir.glob("*.txt")}
    complete_predictions = len(val_ids) == 3769 and prediction_ids == set(val_ids)
    if not complete_predictions:
        raise RuntimeError(
            f"Incomplete MonoDGP prediction set: {len(prediction_ids)}/{len(val_ids)}"
        )

    metrics_dir = output / "official_car_metrics"
    run(
        [
            sys.executable,
            "-u",
    ...
    summary = json.loads((metrics_dir / "kitti_r40_summary.json").read_text(encoding="utf-8"))
    candidate = {
    ...
    for difficulty in ("easy", "moderate", "hard"):
        absolute_difference = abs(candidate[difficulty] - PUBLISHED[difficulty])
        rows.append(
            {
                "difficulty": difficulty,
                "published_car_3d_ap_r40": PUBLISHED[difficulty],
                "reproduced_car_3d_ap_r40": candidate[difficulty],
                "absolute_difference": absolute_difference,
                "max_absolute_difference": MAX_ABS_AP_DIFFERENCE,
                "passed": absolute_difference <= MAX_ABS_AP_DIFFERENCE,
            }
        )
    gate_results = {
        ...
        **{f"car_3d_{row['difficulty']}_within_tolerance": row["passed"] for row in rows},
    }
    ...
    report = {
        ...
        "reproduced_car_3d_ap_r40": candidate,
