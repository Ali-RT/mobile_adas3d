...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate the paired A2c gate.")
    ...
    for name, variant in manifest["variants"].items():
        ...
        rows.append(
            {
                "variant": name,
                "pedestrian_positive_weight": variant["pedestrian_positive_weight"],
                **{key: float(metrics[key]) for key in SOURCE_A2 if "recall" not in key},
    ...
    for row in rows:
        row["pedestrian_near_gain_vs_control"] = (
            row["pedestrian_near_recall"] - control["pedestrian_near_recall"]
        )
        row["vehicle_3d_delta_vs_control"] = (
            row["vehicle_3d_moderate"] - control["vehicle_3d_moderate"]
        )
        row["vehicle_bev_delta_vs_control"] = (
            row["vehicle_bev_moderate"] - control["vehicle_bev_moderate"]
        )
        row["eligible"] = is_eligible(row, control)
    eligible = [row for row in rows if row["eligible"]]
    selected = max(
        eligible,
        key=lambda row: (row["pedestrian_near_recall"], row["mean_3d_moderate"]),
        default=None,
    )
    report = {
        "schema_version": 1,
    ...
    output = output_root / "a2c_gate_comparison.json"
    output.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    with (output_root / "a2c_gate_comparison.csv").open("w", newline="") as handle:
